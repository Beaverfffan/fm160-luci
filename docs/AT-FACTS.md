...
> 连接性的实测判据、NMEA 的实际出口、本机无天线因而无定位、以及各调用的成本。
...
### 10.3 ★ 这台机器**没有接 GNSS 天线**（一条必须写进 UI 的结论）

跨 **100 秒**五次采样（`06:38:12 / 06:38:32 / 06:38:52 / 06:39:12 / 06:39:32`），
响应**逐字节完全相同**，含全部八个校验和，可见卫星恒为 **0**。
...
* **M5 的「有定位」路径在本机无法联调。** 解析器只能按手册 + 本节的空帧形态
  实现，必须**明确标注未经实测**。
...
| 引擎**关**着时读 | `AT+GTGPS?` → `\r\nERROR\r\n` | **必须先开机再读** —— UI 的顺序约束 |
| 开机后**第一次**读 | 载荷 88 字节（只有 `+GTGPS: ` 头，无语句） | **空帧不是错误**，引擎还要一点时间出句子 |
| 开机后第二次起 | 载荷 304 字节（8 条语句） | 稳定形态 |
...
* **无天线**（§10.3）⇒ 有定位形态的 NMEA（含 RMC/GGA）**未经实测**。
