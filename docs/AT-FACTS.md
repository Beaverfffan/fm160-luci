...
* `AT+GTACT?` 里 `auto_seen`（频段值为 `0` = 自动选频）真机**未出现**，`0` 的语义
  ...
  这个判断里最容易搞错的一处。
* `AT+GTCELLLOCK=`（写路径）**未在真机写入**，且当前**没有可写的东西**：
  cell lock 需要 UE 复位才生效，而 FM160 没有传统复位模式；而且无 SIM ⇒ 无
  serving cell ⇒ 没有真实目标可锁，写进去只能是猜的 EARFCN/PCI。
  `AT+GTCELLLOCK=0`（关闭）是唯一内容安全的形式，但现状已是 disabled。
...
| `AT+GTGPSCFG=2,<v>` | **双重**：手册文档集 ∩ 模块 `=?` 答案 | 持久化（掉电保存）= Yes ⇒ 写错**不会被复位抹掉**；而 `AT+GTGPSCFG=?` **从未在真机测过** |

`=?` 从未实测 ⇒ 解析**不假定布局**（收集所有括号组的整数并集），宁可过严。
拿不到答案 ⇒ `caps_valid = false` ⇒ **拒绝写**并记一条机器可见的日志
...
* `AT+GTGPSCFG=?` 的**响应布局** —— 从未测过；解析器按「不假定」处理。
* 有定位形态的 NMEA **全链** —— 由 §11.7 代偿，非真机。
* `AT+GTGPSCFG=2,<v>` 的**写**从未在真机发生（无 `=?` ⇒ 门禁关闭）。
